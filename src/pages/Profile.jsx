...
const fetchProfile=async ()=>{
  try{
     const res=await axios.get(URL+"/blog/v1/user/"+user._id)
...
const handleUserUpdate=async ()=>{
  setUpdated(false)
  try{
    const res=await axios.put(URL+"/blog/v1/user/"+user._id,{username,email,password},{withCredentials:true})
...
const handleUserDelete=async()=>{
  try{
    const res=await axios.delete(URL+"/blog/v1/user/"+user._id,{withCredentials:true})
    setUser(null)
    navigate("/")
    // console.log(res.data)
...
const fetchUserPosts=async ()=>{
  try{
    const res=await axios.get(URL+"/blog/v1/post/user/"+user._id)
    // console.log(res.data)
    setPosts(res.data)
